...
### Step 3: TMS Export to McLeod Format

David wants the bulk lookup results formatted for McLeod LoadMaster import. His TMS expects specific column names, date formats, and status codes that differ from the SearchCarriers API output.

```
Export the bulk lookup results to McLeod CSV format
```

**What happens behind the scenes:**
1. Claude passes the bulk_lookup results to `tms_sync` with `action="export"` and `format="mcleod"`
2. MCP server checks tier: Enterprise required for tms_sync (yes)
3. TMS Mapper loads the McLeod field mapping definition
4. For each carrier: maps fields, converts dates, translates status codes
5. Generates CSV with McLeod-compatible column headers

**Expected output:**

```
TMS EXPORT: McLeod LoadMaster Format
=====================================
Carriers exported: 48
Format: McLeod LoadMaster CSV
Timestamp: 2026-02-26 08:22:41 UTC

CSV CONTENT:
CarrierName,DOTNumber,MCNumber,Address1,City,State,Zip,Phone,Status,SafetyRating,PowerUnits,TotalDrivers,CommonAuthority,ContractAuthority,BrokerAuthority
...
Source: SearchCarriers API v1 | Generated by API Bridge v0.1.0

Copy the CSV content above and save it as a .csv file for McLeod import.
Or specify a file path and I can save it directly.
```

David's analyst copies the CSV, saves it as `carrier_requalification_2026_Q1.csv`, and imports it into McLeod. The process that used to take half a day of manual reformatting took 3 seconds.
...
David now has automated webhook notifications for the two most critical change types -- authority revocations and insurance lapses. His Slack channel will get pinged immediately when any carrier on their panel has a status change, instead of waiting until the next quarterly re-qualification.
...
```
Export carrier data to McLeod format
```
...
The tms_sync tool maps carrier data to TMS-specific formats
(McLeod, TMW, generic CSV) for direct import. It requires
Enterprise tier due to the complex field mapping and format
