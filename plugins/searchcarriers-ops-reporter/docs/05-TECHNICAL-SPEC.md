...
| HTTP client | httpx | 0.27+ | Async HTTP (reserved for future direct data fetching) |
...
Ops Reporter's core formatting logic has zero external dependencies -- it is pure Python string operations. `httpx` is included for future use (planned direct data fetching fallback) but is not used in v0.1. `mcp` provides the MCP server framework.
...
|----------|----------|---------|-------------|
| `SEARCHCARRIERS_API_KEY` | Yes | -- | Bearer token for API authentication. Passed through to upstream plugins. Not used directly by Ops Reporter in v0.1. Format: `{id}\|{token}`. |
...
def format_disclaimer() -> str:
    ...
    return (
        ...
        "Report generated by Ops Reporter v0.1.0*"
    )
