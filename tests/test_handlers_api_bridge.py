...
class TestApiHealth:
    ...
    async def test_filter_by_name(self, fake_api_key):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _api_health(
                {"endpoints": ["search"]}, fake_api_key
            )

        assert len(result["endpoints"]) == 1
        assert result["endpoints"][0]["name"] == "search"
...
class TestBulkLookup:
    ...
    async def test_single_dot_results(self, fake_api_key, carrier_jbhunt):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _bulk_lookup(
                {"dot_numbers": ["299569"]}, fake_api_key
            )
        ...
        assert result["results"][0]["dot_number"] == "299569"
        assert result["results"][0]["success"] is True

    async def test_sections_filter(self, fake_api_key, carrier_jbhunt, authorities_sample):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _bulk_lookup(
                {"dot_numbers": ["299569"], "include": ["basics", "authorities"]},
                fake_api_key,
            )
