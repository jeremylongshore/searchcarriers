...
class TestGenerateReport:
    ...
    async def test_contains_dot_and_name(
        ...
        assert "299569" in result["report"]
        assert "J B HUNT" in result["report"]
    ...
    async def test_format_text_strips_markdown(
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _generate_report(
                {"dot_number": "299569", "format": "text"}, fake_api_key
            )
...
class TestGenerateFleet:
    ...
    async def test_required_keys(
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _generate_fleet({"dot_number": "299569"}, fake_api_key)
    ...
    async def test_fleet_size_dict_structure(
        self, fake_api_key, carrier_jbhunt, equipment_sample
    ):
    ...
    async def test_equipment_type_counts(
        self, fake_api_key, carrier_jbhunt, equipment_sample
    ):
...
class TestGenerateCompare:
    ...
    async def test_fewer_than_2_carriers_error(self, fake_api_key):
        ...
        result = await _generate_compare({"dot_numbers": ["299569"]}, fake_api_key)
    ...
    async def test_required_keys(
        self, fake_api_key, carrier_jbhunt, carrier_werner, authorities_sample, insurances_active
    ):
        """Response has required keys."""
        with respx.mock(base_url=API_BASE) as router:
            # First carrier (JB Hunt)
            ...
            # Second carrier (Werner)
            router.get("/search", params__contains={"dotNumber": "224885"}).mock(
                return_value=httpx.Response(200, json=carrier_werner)
            )
            router.get("/company/224885/authorities").mock(
                return_value=httpx.Response(200, json=authorities_sample)
            )
            router.get("/company/224885/insurances").mock(
                return_value=httpx.Response(200, json=insurances_active)
            )
            result = await _generate_compare(
                {"dot_numbers": ["299569", "224885"]}, fake_api_key
            )
...
class TestExportData:
    ...
    async def test_json_valid(self, fake_api_key, carrier_jbhunt):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _export_data(
                {"dot_number": "299569", "format": "json"}, fake_api_key
            )
    ...
    async def test_markdown_starts_with_heading(self, fake_api_key, carrier_jbhunt):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _export_data(
                {"dot_number": "299569", "format": "markdown"}, fake_api_key
            )

        assert result["format"] == "markdown"
        assert result["data"].startswith("# ")

    async def test_sections_filter(self, fake_api_key, carrier_jbhunt):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _export_data(
                {"dot_number": "299569", "format": "json", "sections": ["basics"]},
                fake_api_key,
            )
    ...
    async def test_invalid_format_defaults(self, fake_api_key, carrier_jbhunt):
