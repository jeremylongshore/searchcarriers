...
def _mock_standard_routes(router, carrier, authorities, insurances, dot="299569"):
...
class TestRiskScore:
    ...
    async def test_required_keys(
        ...
        with respx.mock(base_url=API_BASE) as router:
            _mock_standard_routes(router, carrier_jbhunt, authorities_sample, insurances_active)
            result = await _risk_score({"dot_number": "299569"}, fake_api_key)

        for key in ("dot_number", "risk_score", "risk_level", "factors", "carrier_snapshot", "_pipeline"):
    ...
    async def test_carrier_snapshot_has_legal_name(
        ...
        assert result["carrier_snapshot"]["legalName"] == "J B HUNT TRANSPORT INC"
    ...
    async def test_pipeline_meta(
        ...
        assert result["_pipeline"]["dot_number"] == "299569"
...
class TestVettingCheck:
    ...
    async def test_required_keys(
        ...
        with respx.mock(base_url=API_BASE) as router:
            _mock_standard_routes(router, carrier_jbhunt, authorities_sample, insurances_active)
            result = await _vetting_check({"dot_number": "299569"}, fake_api_key)
    ...
    async def test_verdict_pass_for_authorized(
        ...
        # JB Hunt is authorized, has active authorities and insurance
        assert result["verdict"] in ("PASS", "REVIEW")

    async def test_verdict_fail_no_authority(self, fake_api_key, carrier_jbhunt, insurances_active):
...
class TestInsuranceCheck:
    ...
    async def test_adequate_with_active_policy(self, fake_api_key, insurances_active):
        ...
        with respx.mock(base_url=API_BASE) as router:
            ...
            result = await _insurance_check({"dot_number": "299569"}, fake_api_key)
...
class TestComplianceAudit:
    ...
    async def test_required_keys(
        self, fake_api_key, carrier_jbhunt, authorities_sample
    ):
        """Response has all required keys."""
        with respx.mock(base_url=API_BASE) as router:
            router.get("/search").mock(
            ...
            result = await _compliance_audit({"dot_number": "299569"}, fake_api_key)

        for key in ("dot_number", "checks", "_pipeline"):
            assert key in result, f"Missing key: {key}"

    async def test_check_fields_structure(
        self, fake_api_key, carrier_jbhunt, authorities_sample
    ):
    ...
    async def test_mcs150_pass_when_recent(
        self, fake_api_key, carrier_jbhunt, authorities_sample
    ):
        ...
        mcs_check = next(
            (c for c in result["checks"] if c["check"] == "mcs150_filing"), None
        )
        assert mcs_check is not None
        # JB Hunt fixture has mcs150Date=2025-06-15, which is recent
        assert mcs_check["status"] == "pass"

    async def test_operating_status_pass(
        self, fake_api_key, carrier_jbhunt, authorities_sample
    ):
        ...
        op_check = next(
            (c for c in result["checks"] if c["check"] == "operating_status"), None
        )
