...
class TestSmokeApiBridge:
    ...
    async def test_api_health_full(self, live_api_key, smoke_reports_dir):
        ...
        assert "status" in result, f"Expected 'status' key in result; got keys: {list(result.keys())}"
        ...
        assert "health" in first, f"Endpoint record missing 'health'; got keys: {list(first.keys())}"
    ...
    async def test_api_health_filtered(self, live_api_key, smoke_reports_dir):
        ...
        result = await _api_health({"endpoints": ["search"]}, live_api_key)

        assert_no_error(result)
        assert "endpoints" in result, f"Expected 'endpoints' key; got keys: {list(result.keys())}"
        endpoints = result["endpoints"]
        assert len(endpoints) == 1, (
            f"Expected exactly 1 endpoint for filter ['search']; got {len(endpoints)}"
        )
        assert endpoints[0]["name"] == "search", (
            f"Expected endpoint name 'search'; got {endpoints[0]['name']!r}"
        )
    ...
    async def test_bulk_lookup(self, live_api_key, smoke_reports_dir):
        ...
        dot_numbers = [DOT_JBHUNT, DOT_WERNER]
        ...
        assert isinstance(records, list) and len(records) > 0, (
            "Expected at least one result record"
        )
